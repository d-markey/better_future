...
void main() async {
  try {
    final workloads = {
      'L': () => compute('L'), // independent async task
    ...
    final outcomes = await BetterFuture.settle<String>(workloads);

    report(
      'Result Map: '
      '${outcomes.entries.map((e) => '${e.key}: ${e.value.runtimeType}')}',
    );
    final finalResult = outcomes['final_result']!;
    switch (finalResult) {
      case BetterSuccess():
        print('[${sw.elapsed}] Final String: ${finalResult.result}');
      case BetterFailure():
        print('[${sw.elapsed}] Final String: **ERROR: ${finalResult.error}**');
    }
  } catch (ex) {
    print('[${sw.elapsed}] Caught expected error: $ex');
  }
}
...
Future<String> compute(String id) async {
  ...
  if (rnd.nextInt(10) < 2) throw Exception('Random error!');
  return id;
}
